...
async def read_keywords(project_id: Optional[uuid.UUID] = None, db: AsyncSession = Depends(get_db)):
    return await service.get_keywords(db=db, project_id=project_id)

@router.get("/{id}", response_model=KeywordResponse)
async def read_keyword(id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def update_keyword(id: uuid.UUID, keyword_in: KeywordUpdate, db: AsyncSession = Depends(get_db)):
    keyword = await service.get_keyword(db=db, keyword_id=id)
    if not keyword:
        raise HTTPException(status_code=404, detail="Keyword not found")
...
async def delete_keyword(id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    keyword = await service.get_keyword(db=db, keyword_id=id)
    if not keyword:
        raise HTTPException(status_code=404, detail="Keyword not found")
